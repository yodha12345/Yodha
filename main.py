...
def load_questions():
    ...
    for sub in subjects:
        ...
        try:
            ...
            # Normalize Line Endings and split into blocks safely
...
def run_quiz(chat_id):
    ...
    if sub in question_bank:
        ...
        # Global Fallback if chapter title lookup misses
        if not all_pool:
            for ch in question_bank[sub].keys():
                all_pool.extend(question_bank[sub][ch])
    
    if not all_pool:
        bot.send_message(chat_id, f"❌ No questions could be found inside your `{sub}.txt` file!")
        return
    ...
    for block in selected:
        ...
        try:
            ...
            # Strip Question numbers from the question text header dynamically if present
            if q_text[0].isdigit():
                parts = q_text.split(".", 1) if "." in q_text else q_text.split(" ", 1)
                if len(parts) > 1:
