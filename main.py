...
def init_db():
    ...
    c = conn.cursor()
    # [span_6](start_span)History for 30-day repetition logic[span_6](end_span)
    c.execute('''CREATE TABLE IF NOT EXISTS history 
                 (question_hash TEXT PRIMARY KEY, last_used TIMESTAMP)''')
    # [span_7](start_span)[span_8](start_span)All-Time Stats[span_7](end_span)[span_8](end_span)
    ...
    # [span_9](start_span)[span_10](start_span)Weak Points by Chapter[span_9](end_span)[span_10](end_span)
...
def mark_question_used(question_text):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    # [span_11](start_span)Use MD5 hash to uniquely identify questions[span_11](end_span)
...
def get_used_hashes_30_days():
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    [span_12](start_span)thirty_days_ago = datetime.now() - timedelta(days=30) # 30-day logic[span_12](end_span)
...
def save_session_to_db(session_scores, chapters_map):
    ...
    for uid, stats in session_scores.items():
        # [span_13](start_span)[span_14](start_span)Update All-Time Table[span_13](end_span)[span_14](end_span)
        ...
        # [span_15](start_span)[span_16](start_span)Track chapter-wise weak points[span_15](end_span)[span_16](end_span)
...
quiz_active = {} 
skipped_this_q = set() 
wrong_chapters_tracker = {} # Temporary session tracker for chapters
...
def handle_poll_answer(poll_answer):
    ...
    if str(poll_answer.poll_id) == str(current_poll_data["poll_id"]):
        ...
        if poll_answer.option_ids[0] == current_poll_data["correct_id"]:
            ...
            [span_17](start_span)user_scores[uid]["score"] -= 0.25 # Negative marking[span_17](end_span)
            # [span_18](start_span)Track which chapter they got wrong[span_18](end_span)
...
def run_quiz(chat_id):
    ...
    # [span_19](start_span)[span_20](start_span)30-Day Non-Repeat Logic[span_19](end_span)[span_20](end_span)
    all_pool = []
    ...
    # [span_21](start_span)If not enough fresh questions, fallback to all but warn[span_21](end_span)
    ...
    for block in selected:
        ...
        [span_22](start_span)mark_question_used(block) # Log to 30-day history[span_22](end_span)
        ...
        [span_23](start_span)current_poll_data["chapter"] = ch_name # Track the chapter[span_23](end_span)
        ...
        try:
            ...
            [span_24](start_span)bot.stop_poll(GROUP_ID, poll_msg.message_id) # Close poll logic[span_24](end_span)
        except: pass
        time.sleep(2)

    [span_25](start_span)save_session_to_db(user_scores, wrong_chapters_tracker) # Store All-Time memory[span_25](end_span)
    
    # [span_26](start_span)Final Report[span_26](end_span)
...
# [The rest of your load_questions() and admin handlers remain the same]
